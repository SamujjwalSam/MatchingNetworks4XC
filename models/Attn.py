...
class Attn(nn.Module):
    ...
    def forward(self, similarities, support_set_y, dim=1):
        ...
        # logger.debug(support_set_y.shape)
        softmax_similarities_unsq = softmax_similarities.unsqueeze(1)
        logger.debug("softmax_similarities_unsq.shape: {}".format(softmax_similarities_unsq.shape))
        support_set_y_bmm = softmax_similarities_unsq.bmm(support_set_y)
