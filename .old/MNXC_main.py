...
def read_fastai_csv(dataset_path="D:\\Datasets\\nlp", dataset_name="ag_news_csv", file_name="train.csv", tag="train"):
    ...
    import csv
    rows = []
    with open(join(dataset_path, dataset_name, file_name)) as csvfile:
...
def download_fastai_dataset(config, dataset='ag_news_csv'):
    ...
    :return:
    """
    # logger.debug(config["paths"]["dataset_url"], join(config["paths"]["dataset_dir"], config["paths"]["dataset_name"]))
    data = untar_data(url=config["paths"]["dataset_url"],
                      fname=join(config["paths"]["dataset_dir"][plat],
...
def prepare_datasets(config, dataset='ag_news_csv'):
    ...
    if dataset in config["fastai_datasets"]:
        ...
        labels = OrderedDict()
        with open(join(fastai_path, dataset, "classes.txt")) as cls:
...
def main(args):
    ...
    cls = Run_Network(data_loader, use_cuda=use_cuda)
